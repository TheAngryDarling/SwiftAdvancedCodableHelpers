...
public extension SingleValueDecodingContainer {
    ...
    func decodeDictionary<D>(excludingKeys: [D.Key] = [],
    ...
    func decodeArray(customDecoding: (_ decoder: Decoder) throws -> Any? = { _ in return nil }) throws -> Array<Any>  {
        let catcher = try self.decode(DecoderCatcher.self)
...
public extension SingleValueDecodingContainer {
    ...
    ///   - customDecoding: Custom decoding of object type
    /// - Returns: Returns an array of elements that decoded or nil if no key found
    func decodeFromSingleOrArrayIfPresent<Element>(customDecoding: (_ decoder: Decoder) throws -> Element) throws -> [Element]? where Element: Decodable {
        guard !self.decodeNil() else { return nil }
    ...
    func decodeFromSingleOrArrayIfPresentWithEmptyDefault<Element>(customDecoding: (_ decoder: Decoder) throws -> Element) throws -> [Element] where Element: Decodable {
